...
_Q15 hard_clipping(_Q15 sample, unsigned int parameter_val)
{
    ...
    //pre-filter
    in = DF2SOStructure(in, &hard_clipping_dat[3], &hard_clipping_dat[0], hard_clipping_prefilter_buf, &hard_clipping_prefilter_buf[2]);
...
_Q15 soft_clipping(_Q15 sample, unsigned int parameter_val)
{
    ...
    //pre-filter
    in = DF2SOStructure(in, &soft_clipping_dat[3], &soft_clipping_dat[0], soft_clipping_prefilter_buf, &soft_clipping_prefilter_buf[2]);

    if(in >= 0)
    {
        in = Q16mpy(in, soft_clipping_dat[parameter_val + 6]);

        if (Q16abs(in) < Q16ftoi(0.3333))
            out = Q16mpy(in, Q16ftoi(2.0));

        if (Q16abs(in) >= Q16ftoi(0.3333))//y(n) = (3-(2-x(n)*3).^2)/3
            out = Q16mpy((Q16ftoi(3.0) - Q16pow(Q16ftoi(2.0) - Q16mpy(in, Q16ftoi(3.0)),2)), Q16ftoi(0.3333));

        if (Q16abs(in) > Q16mpy(Q16ftoi(2.0), Q16ftoi(0.3333)))
            out = Q16ftoi(1.0);
    }
    else
        out = in;
...
_Q15 compression(_Q15 sample, unsigned int parameter_val)
{
    ...
    _Q16 TAV = compression_dat[parameter_val];

    e = compression_buf + (Q16pow(in, 2) - compression_buf)*TAV;
    compression_buf = e;

    out = (Q16ftoi(1.0) - e)*in;
...
{
    delay_effects_buf = (_Q15*)malloc(2*DELAY_BUF_LEN);
    if(delay_effects_buf == NULL)
        while(1);//ERROR STATE
    mod_effects_buf0 = (_Q15*)malloc(2*CHORUS_BUF_LEN);
    if(mod_effects_buf0 == NULL)
        while(1);//ERROR STATE
    mod_effects_buf1 = (_Q15*)malloc(2*CHORUS_BUF_LEN);
    if(mod_effects_buf1 == NULL)
        while(1);//ERROR STATE
    panoram_buf = (_Q15*)malloc(2*400);
    if(panoram_buf == NULL)
        while(1);//ERROR STATE
}
