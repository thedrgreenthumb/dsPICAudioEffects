...
inline _Q15 Q15mpy(_Q15 a, _Q15 b)
{
	...
	//Add saturation flag setup for accA, see manual
...
inline _Q16 Q16mpy(_Q16 a, _Q16 b)
{
    _Q16 c = 0;
	//Here a lot of sheat
	//This multiplication must be saturated
    if((a < 0) && (b >= 0))
    {
        a = _Q16neg(a);
        c = _Q16mac(a, b, 0);
...
{
	//Check again for negative numbers, the _Q16mpy might not work, because this  
   return x << 1;
}
